...
export const querysUsers = {
  ...
  validateToken: "SELECT * FROM Usuario WHERE Token = @Token" 
};  
...
export const querysCitas = {
  ...
  getAllCitasDay: "SELECT IdCita, IdUser, IdDependencia, Citas.idPaciente, HorarioInicio , HoraFin , Descripcion, Estado, Nombre, ApellidoP,ApellidoM FROM Citas INNER JOIN Paciente ON Citas.idPaciente = Paciente.IdPaciente WHERE CAST(HorarioInicio AT TIME ZONE 'UTC' AT TIME ZONE 'Canada Central Standard Time' AS DATE) = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'Canada Central Standard Time' AS DATE) ORDER BY HorarioInicio;",
  getAllCitasDayHour: "SELECT IdCita, IdUser, IdDependencia, Citas.idPaciente, HorarioInicio, HoraFin, Descripcion, Estado, Nombre, ApellidoP,ApellidoM FROM Citas INNER JOIN Paciente ON Citas.idPaciente = Paciente.IdPaciente WHERE CAST(HorarioInicio AS DATE) = CAST(GETDATE() AS DATE) AND CAST(HorarioInicio AS TIME) >  CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'Canada Central Standard Time' AS TIME) order by HorarioInicio ",
  getCitasByDayOfWeek: "SELECT IdCita, IdUser, IdDependencia, Citas.idPaciente, HorarioInicio, HoraFin, Descripcion, Estado, Nombre, ApellidoP, ApellidoM FROM Citas INNER JOIN Paciente ON Citas.idPaciente = Paciente.IdPaciente WHERE DATEPART(dw, HorarioInicio) = @numeroDia order by HorarioInicio",
